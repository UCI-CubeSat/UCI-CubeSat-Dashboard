...
export default function Dashboard() {
  ...
  const [queryParams, setQueryParams] = useState<DashboardQueryParam | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    try {
      const validatedQueryParams =
        DashboardQueryParamValidator.parse(currentQueryParams);
      setQueryParams(validatedQueryParams);
    } catch (e) {
      navigate(
        location.pathname + "/?" + encodeGetParams(DEFAULT_QUERY_PARAMS)
      );
    }
  }, [location.search]);
...
const DashboardContent = (props: DashboardContentProps) => {
    ...
    queryFn: async () => {
      ...
      if (auth !== null) {
        if (props.queryParams.mode === "offset") {
          return await satellitesByOffset({
            pageNo: props.queryParams.pageNo,
            count: props.queryParams.count,
            token: auth.token,
          });
        } else {
          return await satellitesByTimeRange({
            start: props.queryParams.start,
            end: props.queryParams.end,
            token: auth.token,
          });
        }
      } else {
  ...
  const numLogsAfter =
    response.data?.type === "offset" ? response.data.numLogsAfter : null;
  const numLogsBefore =
    response.data?.type === "offset" ? response.data.numLogsBefore : null;
  return (
    <Grid
      container
      style={{
